...
plot(impulseFFR2)
```
IRFs appear to be somewhat comparable to before. However, they appear to be less smooth than in the initial model.
...
We chose 2 different sample split periods: 1985-2020 and 1955-2007. The rationale behind splitting the data at 1985 was that the time series of the GDP deflator seems to be becoming stationary after this point, which would enable us to avoid taking second differences and solely work with first differences. On the other hand, the rationale for splitting the sample at 2007 was to avoid the period of the Global Financial Crisis (GFC) when much unconventional monetary policy was employed, which could make identification of monetary policy responses problematic.

First we split the data at 1985:
...
```
The IRFs for this period appear to respond quite differently from the whole sample: no negative impact on the output gap. Due to not taking second differences, the unit of measurement for inflation has now changed (measured in percentage points). Interestingly, inflation does not become negative and spikes up positively in the first periods. 
